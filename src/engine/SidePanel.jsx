...
function cleanStaleEntities(sim) {
  ...
  for (const [id, order] of Object.entries(orders)) {
    ...
    const driver          = drivers[order.driver_id];
    const driverMissing   = order.driver_id && !driver;
    const customerMissing = !customers[order.customer_id];

    if (driverMissing) {
      const now = sim?.engine?._simTime ?? 0;
      const reconnectWindow = sim?.engine?._getParam?.('reconnect_window_s', 600);

      if (order.status === 'on_the_way') {
        if (!order._reconnect_deadline) {
          order._reconnect_deadline = now + reconnectWindow;
        }

        const expired = now > order._reconnect_deadline;

        if (expired) {
          world.driver_penalties = world.driver_penalties ?? {};
          world.driver_penalties[order._last_driver_name] =
          (world.driver_penalties[order._last_driver_name] ?? 0) + 1;
        ...
        // Dentro de ventana — no tocar la orden, esperar reconexión
        continue;
      }

      // assigned — más tolerante, reencolar directo
