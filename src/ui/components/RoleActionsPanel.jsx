...
export default function RoleActionsPanel({ sim }) {
  ...
  const driverOrders = useMemo(() => {
    if (!driverId) return [];
    return orders.filter(order => order.driver_id === driverId || !order.driver_id || order.status === 'queued');
  }, [orders, driverId]);
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto' }}>
      <Section title="Driver" color="var(--driver-color)">
        <Selector
      ...
      <Section title="Cliente" color="var(--customer-color)">
        <Selector
          ...
          options={customers.map(customer => ({ value: customer.id, label: `${customer.name} · max ${customer.max_distance_km}km` }))}
        ...
        <MiniState>
          {selectedCustomer ? (
            <>
              ...
              <MiniItem label="Distancia máx" value={`${selectedCustomer.max_distance_km}km`} />
            </>
      ...
      <Section title="Engine" color="var(--accent)">
        <div style={{ fontSize: 11, color: 'var(--text-1)', marginBottom: 10 }}>
          Inspirado en el dashboard admin de Morelivery: aquí puedes ajustar una selección útil de parámetros del motor sin salir del simulador.
        </div>
...
function Section({ title, color, children }) {
  return (
    <div className="panel-section">
      ...
      <div className="panel-section-body" style={{ padding: 12 }}>
        {children}
      </div>
    </div>
  );
